### Kent Mouw Radiation-associated bladder cancer cohort of patients with prior XRT for prostate cancer 
### Feb 2021 
### Initial cohort had 36 samples, but 9 were excluded:
## 7 samples didn't pass ContEst < 0.04: BS06T08299, BS09E11679, BS14DO1686, BS16E70362, BS16X56520, BS17E40708, S04027690
## 1 sample was histologic prostate cancer (BS17N58375). 1 sample was from a patient received radiation for rectal ca, not prostate cancer BS14W49540
...
#path to Mouw Rad associated BCa MAF files
...
dim(df_reduced) # 17046   329


### ============================================================================
#read Mouw cohort clinical info
#rt_bca_clin <- read.csv("mouwclinicfinal.csv", header = TRUE)
#str(rt_bca_clin)

### ============================================================================
## CoMut MIBC TCGA genes in MafTools
## Maff tools
...
## Export the dataframe as a tab txt file
...
## Plot FGFR3 in comut
cna_fgfr3_mouw <- cna_mouw %>%
        dplyr::slice(8)
write.table(cna_fgfr3_mouw, file= "bladder_fgfr3.csv", sep = "\t", row.names = FALSE, quote = FALSE)


## =============================================================================
## Mutational signatures from maftools
...
############################################### Analyze Mutations in Mouw cohort and TCGA
...
# count variants for results section of the paper
...
# mean(tcga_indel_variants[["ratio"]]) # 0.02367367
# mean(mouw_indel_variants[["ratio"]]) # 0.04848898
...
#mean(tmb_tcga[["tmb"]]) #  8.086294 for the result section of the paper
#median(tmb_tcga[["tmb"]])
#summary(tmb_tcga[["tmb"]])

tmb_mouw <- dplyr::rename(tmb_mouw,case.id = Tumor_Sample_Barcode)
tmb_mouw$case.id[c(1:27)] <- "RA BC"
#mean(tmb_mouw[["tmb"]]) # 11.10617 for the result section of the paper
#median(tmb_mouw[["tmb"]]) # 9.466667
#summary(tmb_mouw[["tmb"]]) # Q1 7.283; Q3 14.133
...
# Figure with all ratio and tmb plots together
ggarrange(plotsnvratio, plotindels, 
          ncol = 2, nrow = 1, 
          common.legend = TRUE, legend = "right")

##########################################################################################################################
## plot MIBC mutated genes in TCGA vs Mouw. Answer the question: Does this cohort resembles MIBC?
...
## plot mutated DNA repair genes in TCGA vs Mouw. Given presence Sig6 in Mouws cohort, are there higher frequency of mutation in Mouw vs TCGA?
...
#ggarrange(plottp53, plotrb1, ploterbb2, plotfanca, plotmsh6, plotchek2, 
#          ncol = 3, nrow = 2, 
#          common.legend = TRUE, legend = 'right')
...
#dim(mut_mibcgenes_mouw_plot) 
#count(mut_mibcgenes_mouw[mut_mibcgenes_mouw$Hugo_Symbol== c("TP53"), ]) #22
